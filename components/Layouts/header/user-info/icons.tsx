...
export function MenuItem({
  ...
  // The shared classes for both Links and Buttons
  const baseClasses = cn(
    "group relative flex w-full items-center gap-3 rounded-xl px-3 py-2.5 outline-none transition-all duration-300",

    // Inactive State: Subtle hover effects
    !isActive && [
      "text-slate-500 dark:text-slate-400",
      "hover:bg-slate-100/80 hover:text-slate-900",
      "dark:hover:bg-white/[0.04] dark:hover:text-slate-200"
    ],

    // Active State: Glowing Cyan HUD effect
    isActive && [
      "font-semibold text-sky-600 dark:text-sky-400",
      "bg-sky-50 dark:bg-sky-500/10",
      "shadow-sm dark:shadow-[inset_0_0_20px_rgba(56,189,248,0.05)]",
    ],
    className
  );

  // The glowing indicator bar that appears on the left side of active items
  const ActiveIndicator = isActive ? (
    <span
      className="absolute left-0 top-1/2 h-3/5 w-1 -translate-y-1/2 rounded-r-full bg-sky-500 shadow-[0_0_10px_rgba(56,189,248,0.6)]"
      aria-hidden="true"
    />
  ) : null;
  ...
  if (as === "link") {
    return (
      <Link href={href} className={baseClasses} onClick={onClick}>
        {ActiveIndicator}
        {/* We wrap children to ensure the glowing text color cascades to any icons passed in */}
        <span className="flex w-full items-center gap-3 transition-colors duration-300 group-hover:text-sky-500 dark:group-hover:text-sky-400">
          {children}
        </span>
      </Link>
    );
